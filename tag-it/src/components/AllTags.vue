<template>
    <h5>ALL TASK LIST</h5>
    <p>
        reordering of the tasks? yet to enable filter. set specific colors for
        ...
        <div id="padding">
            ...
            <table id="AllTagsTable" class="auto-index">
                <tr>
                    ...
                    <th>
                        <div class="filter-container">
                            ...
                            <i @click="filterTitle">
                                ...
                                <BIconSortAlphaDown
                                    v-if="
                                        this.filterSettings.sortState === 'none'
                                    "
                                />
                            </i>
                        </div>
                    ...
                    <th>
                        <div class="filter-container">
                            ...
                            <i @click="filterStartDate">
                                <BIconSortDown
                                    v-if="!this.filterSettings.filterStartDate"
                                />
                                <BIconSortDown
                                    v-if="this.filterSettings.filterStartDate"
                                />
                    ...
                    <th>
                        <div class="filter-container">
                            ...
                            <i @click="filterEndDate">
                                <BIconSortDown
                                    v-if="!this.filterSettings.filterEndDate"
                                />
                                <BIconSortDown
                                    v-if="this.filterSettings.filterEndDate"
                                />
                    ...
                    <th>
                        <div class="filter-container">
                            <i @click="filterImpt">
                                <BIconFlag
                                    v-if="!this.filterSettings.filterImpt"
                                />
                                <BIconFlagFill
                                    v-if="this.filterSettings.filterImpt"
                                />
                ...
                <tr v-for="(row, index) in tableRows" :key="row.tag_id">
                    ...
                    <td>{{ row.end }}</td>
                    <td>
                        <i @click="flagbutton(row.tag_id, row.flagged)">
...
export default {
    ...
    methods: {
        ...
        async filterStartDate() {
            ...
            this.filterSettings.filterStartDate =
                !this.filterSettings.filterStartDate;
        ...
        async filterEndDate() {
            let filteredRows = Array.from(this.originalRows);
            this.filterSettings.filterEndDate =
                !this.filterSettings.filterEndDate;
        ...
        async getCal(event, cal_id) {
            ...
            if (isChecked) {
                ...
                this.filterCals = this.filterCals.filter(
                    (item) => item !== cal_id
                );
                this.loadCals();
            }
                ...
                for (let i = 0; i < this.filterCals.length; i++) {
                    ...
                    for (let i = 0; i < tags.length; i++) {
                        ...
                        if (tagDoc.exists()) {
                            ...
                            console.log(
                                "Document does not exist for tag:",
                                tags[i]
                            );
                        }
                    }
        ...
        async flagbutton(tag_id, flagged_status) {
            //when synced with firestore, change to async + put in export default? under mount?
        ...
        async deleteTag(tag_id) {
            ...
            try {
                ...
                if (tagDoc.exists()) {
                    ...
                    if (calendarId) {
                        ...
                        if (
                            calDelTags.length === 0 &&
                            calendarId !== this.miscCal_id &&
                            calUsers.length === 1
                        ) {
                            const userRef = doc(db, "User", this.user_id);
                            await updateDoc(userRef, {
                                [`personal_calendars.${calendarId}`]:
                                    deleteField(),
                            });
            ...
            await this.getUserCalendars();
            console.log(
                "Successfully deleted tag and updated calendar!",
                tag_id
            );
        },
        ...
        async fetchAndUpdateData() {
            ...
            if (miscTags && Array.isArray(miscTags)) {
                miscTags.forEach((tagId) => {
                    let tagPromise = getDoc(doc(db, "Tags", tagId)).then(
                        (tagDoc) => {
            ...
            for (let i = 0; i < this.cals.length; i++) {
                let calDoc = await getDoc(
                    doc(db, "Calendar", this.cals[i]["cal_id"])
                ); // fetch calendar doc via calendar cal_id
                let calTags = calDoc.data().tags; // get array of tag_id's of calendar
                if (calTags && Array.isArray(calTags)) {
                    // ensure that array of tag_id's is not empty
