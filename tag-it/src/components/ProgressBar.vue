    ...
    methods: {
                    ...
                    if (cal_tags.length > 0) {
                        await Promise.all(
                            cal_tags.map(async (tagId) => {
                                ...
                                if (tagCompleted) { this.calID_completed[calId] += 1 }
                            }))
                    }
                })
            )
        ...
        async generateChart() {
            ...
            const progressData = this.user_calendars.map(cal_id => ({
                    ...
                    percent: parseFloat(((this.calID_completed[cal_id] / this.calID_total[cal_id]) * 100).toFixed(1)),
                        ...
                        datasets: [
                            {
                                ...
                                backgroundColor: progressData.map(row => row.color),
                                borderSkipped: false,
                                borderRadius: 10,
                                barPercentage: 0.2,
